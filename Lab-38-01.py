...
# Forward/backward speed (Tello accepts -100 to 100)
SPEED_FB = 20


def adjust_tello_position(offset_z):
    """
    Move forward/backward using send_rc_control for real-time non-blocking control.
    :param offset_z: Area (w * h) of the detected face rectangle
    """
    if offset_z == 0:
        tello.send_rc_control(0, 0, 0, 0)  # no face → stop
        return
    ...
    if offset_z < TARGET_MIN:
        ...
        fb = 0           # within target range → stop

    # lr=0, fb=fb, ud=0, yaw=0 (non-blocking, sent every frame)
