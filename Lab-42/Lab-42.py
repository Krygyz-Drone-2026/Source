...
BUFFER_LEN             = 10   # GestureBuffer: confidence window size
HISTORY_LEN            = 16   # GestureRecognition: point history length (model input = 16×2 = 32)
...
class KeyPointClassifier:
    def __init__(self, num_threads=1):
        ...
        # Use model_content (bytes) instead of model_path to avoid
        # Unicode/spaces in path causing ValueError on Windows
        with open(model_path, 'rb') as f:
            model_data = f.read()
        self.interpreter = TFLiteInterpreter(model_content=model_data,
...
class TelloKeyboardController:
    ...
    def control(self, key):
        ...
        if   key == ord('w'): print('self.tello.move_forward(30)')
        elif key == ord('s'): print('self.tello.move_back(30)')
        elif key == ord('a'): print('self.tello.move_left(30)')
        elif key == ord('d'): print('self.tello.move_right(30)')
        elif key == ord('e'): print('self.tello.rotate_clockwise(30)')
        elif key == ord('q'): print('self.tello.rotate_counter_clockwise(30)')
        elif key == ord('r'): print('self.tello.move_up(30)')
        elif key == ord('f'): print('self.tello.move_down(30)')
...
def main():
    ...
    gesture_detector = GestureRecognition(
        ...
        history_length=HISTORY_LEN,   # must match model input: 16×2=32
    )
